      ...
      if (data.error) {
        ...
        $('.points-allocated-transactions').html(function() {
          ...
          for (var i = 0; i < transactionData.length; i++) {
            str = str + '<p>timeStamp: ' + transactionData[i].timestamp + '<br />partner: ' + transactionData[i].partner + '<br />member: ' + transactionData[i].member + '<br />points: ' + transactionData[i].points + '<br />level: ' + transactionData[i].level + '<br />transactionID: ' + transactionData[i].transactionId + '</p><br>';
          }
          return str;
        });
...
$('.earn-points-electricity').click(function() {

  var formPoints = $('.electricity input').val();
  if(formPoints<25){
    formpoints=150;
  }
  else if(formPoints<50){
    formPoints=100;
  }
  else if(formPoints<75){
    formPoints=75;
  }
  else{
    formPoints=0;
